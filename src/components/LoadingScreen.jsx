 ...
 *
 * Props:
 * - entered: boolean (true when parent has pressed Ready? — triggers fade-out)
 * - onEnter(): called when user clicks "Ready?"
 * - duration (ms): how long the progress bar visually takes (default 2000)
 */
export default function LoadingScreen({ entered = false, onEnter, duration = 500 }) {
  ...
  // if parent resets `entered` to false, make sure overlay becomes visible again
  useEffect(() => {
  ...
  // Keep mounted while fading; when the fade animation finishes we hide it
  ...
  return (
    ...
    >
      {/* Inline styles block — drop-in, no extra CSS file needed */}
      <style>{`
        .loader-overlay{
          position:fixed;
          inset:0;
        ...
        /* sphere + rings breathing glow */
        .sphere {
          filter: drop-shadow(0 0 30px rgba(150, 200, 255, 0.8))
                  drop-shadow(0 0 60px rgba(180, 220, 255, 0.5));
          ...
          background: radial-gradient(circle at 30% 30%, rgba(190,220,255,0.25), transparent 60%);
          border-radius: 50%;
        }
        @keyframes pulseGlow {
          0%, 100% { transform: scale(1); filter: drop-shadow(0 0 30px rgba(150, 200, 255, 0.8)); }
          50% { transform: scale(1.05); filter: drop-shadow(0 0 50px rgba(150, 200, 255, 1)); }
        }
        ...
        .ring {
          ...
          background: radial-gradient(circle at 30% 20%, rgba(40,161,252,0.08), transparent 10%);
        }
        ...
        .progress-fill{
          height: 100%;
          width: 0%;
          background: linear-gradient(90deg, rgba(180,220,255,1), rgba(120,160,255,0.95));
          transition: width 300ms cubic-bezier(0.45, 0.05, 0.55, 0.95);
          box-shadow: 0 6px 18px rgba(120,160,255,0.25);
